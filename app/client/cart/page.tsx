...
export default function CartPage() {
  ...
  // Sync selectedIds khi items thay đổi:
  // - giữ các item đã chọn còn trong cart
  // - tự chọn item mới thêm vào
  useEffect(() => {
    setSelectedIds((prev) => {
      const currentIds = new Set(items.map((i) => i.id));
      const next = new Set<string>();
      items.forEach((item) => {
        // item mới (chưa có trong prev) → chọn mặc định
        if (!prev.size || prev.has(item.id) || !currentIds.has(item.id)) {
          next.add(item.id);
        }
      });
      // giữ lại selection đã có nếu item vẫn còn trong cart
      prev.forEach((id) => {
        ...
        <div className="mx-auto max-w-7xl px-4 md:px-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-3">
              {/* Select all row — chỉ hiện khi đăng nhập */}
              {user && (
              ...
              {items.map((item) => {
                ...
                return (
                  <div
                    key={item.id}
                    className={`flex gap-4 p-4 bg-card rounded-lg border shadow-sm transition-colors ${
                      user && !isSelected ? "opacity-50" : ""
                    }`}
                  >
                    {/* Checkbox — chỉ hiện khi đăng nhập */}
                    {user && (
            ...
            <div className="lg:col-span-1">
              <div className="bg-card rounded-lg border shadow-sm p-6 sticky top-4">
                ...
                <Button
                  ...
                  size="lg"
                  disabled={user ? selectedIds.size === 0 : false}
